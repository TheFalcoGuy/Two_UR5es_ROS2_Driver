...
static size_t _ListControllerTypes_Request__max_serialized_size(bool & full_bounded)
{
  return max_serialized_size_controller_manager_msgs__srv__ListControllerTypes_Request(
    full_bounded, 0);
}
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: base_classes
  {
    ...
    if (!rosidl_runtime_c__String__Sequence__init(&ros_message->base_classes, size)) {
      return "failed to create array for field 'base_classes'";
    }
    auto array_ptr = ros_message->base_classes.data;
    for (size_t i = 0; i < size; ++i) {
      std::string tmp;
...
static size_t _ListControllerTypes_Response__max_serialized_size(bool & full_bounded)
{
  return max_serialized_size_controller_manager_msgs__srv__ListControllerTypes_Response(
    full_bounded, 0);
}
