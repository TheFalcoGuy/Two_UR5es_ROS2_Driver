  ...
  void * untyped_ros_message)
{
  ...
  // Field name: interface_names
  {
    ...
    if (!rosidl_runtime_c__String__Sequence__init(&ros_message->interface_names, size)) {
      return "failed to create array for field 'interface_names'";
    }
    auto array_ptr = ros_message->interface_names.data;
    for (size_t i = 0; i < size; ++i) {
      std::string tmp;
...
static size_t _InterfaceValue__max_serialized_size(bool & full_bounded)
{
  return max_serialized_size_control_msgs__msg__InterfaceValue(
    full_bounded, 0);
}
