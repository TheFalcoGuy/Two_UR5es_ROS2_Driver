...
class UrRobotiqGripper ():
    ...
    def is_active(self):
        ...
        return UrRobotiqGripper.GripperStatus(status) == UrRobotiqGripper.GripperStatus.ACTIVE
    ...
    def auto_calibrate(self, log: bool = True) -> None:
        ...
        if UrRobotiqGripper.ObjectStatus(status) != UrRobotiqGripper.ObjectStatus.AT_DEST:
    ...
    def goTo(self, position: int, speed=255, force=255) -> Tuple[bool, int]:
        """Sends commands to start moving towards the given position, with the specified speed and force.
        :param position: Position to move to [min_position, max_position]. Only accepts vals between 0 and 255
    ...
    def move_and_wait_for_pos(self, position: int, speed: int, force: int) -> Tuple[int, ObjectStatus]:  # noqa
        ...
        """
        set_ok, cmd_pos = self.goTo(position, speed, force)
        if not set_ok:
        ...
        while UrRobotiqGripper.ObjectStatus(cur_obj) == UrRobotiqGripper.ObjectStatus.MOVING:
            cur_obj = self._get_var(self.OBJ)

        # report the actual position and the object status
        final_pos = self._get_var(self.POS)
        final_obj = cur_obj
        return final_pos, UrRobotiqGripper.ObjectStatus(final_obj)


# def main():
#     # #Socket setings
#     HOST="192.168.0.88" #replace by the IP address of the UR robot
...
#     gripper = ur_robotiq_gripper.UrRobotiqGripper()
