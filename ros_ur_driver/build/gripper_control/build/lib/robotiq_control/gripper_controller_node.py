...
class GripperController(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('comm_method', 'ur')
        comm_method = self.get_parameter('comm_method').get_parameter_value().string_value
        print("comm_methd %s"  % comm_method)
        if comm_method == "ur":
            self.grip = UrRobotiqGripper()
            HOST="192.168.0.88" #TODO: replace by the IP address of the UR robot
            PORT=63352 #PORT used by robotiq gripper
            self.get_logger().info("Connecting to ur...")
            self.grip.connect(HOST, PORT)
